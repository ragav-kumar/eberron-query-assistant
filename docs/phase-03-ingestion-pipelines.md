...
- Persist normalized sources and chunks in the authoritative storage model defined by the final-state spec.
...
At the end of this phase, the application can ingest all supported source classes and persist normalized corpus data with enough metadata to support later retrieval and citation. Startup decisions from Phase 02 now drive real ingestion work instead of only reporting what would happen.
