...
const getArticleCachePath = (config: RuntimeConfig, url: string): string => path.join(config.cacheDir, 'keith-baker', `${hashUrl(url)}.html`);

const hashUrl = (url: string): string => createHash('sha256').update(url).digest('hex').slice(0, 32);
